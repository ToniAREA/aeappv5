    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.documentations.update", [$documentation->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $documentation->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id">
                    @foreach($categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $documentation->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('expiration_date') ? 'is-invalid' : '' }}" type="text" name="expiration_date" id="expiration_date" value="{{ old('expiration_date', $documentation->expiration_date) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_valid') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_valid" id="is_valid" value="1" {{ $documentation->is_valid || old('is_valid', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" type="text" name="notes" id="notes" value="{{ old('notes', $documentation->notes) }}">
                @if($errors->has('notes'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('internal_notes') ? 'is-invalid' : '' }}" type="text" name="internal_notes" id="internal_notes" value="{{ old('internal_notes', $documentation->internal_notes) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $documentation->link) }}">
                @if($errors->has('link'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link_description') ? 'is-invalid' : '' }}" type="text" name="link_description" id="link_description" value="{{ old('link_description', $documentation->link_description) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('authorized_roles') ? 'is-invalid' : '' }}" name="authorized_roles[]" id="authorized_roles" multiple>
                    @foreach($authorized_roles as $id => $authorized_role)
                        <option value="{{ $id }}" {{ (in_array($id, old('authorized_roles', [])) || $documentation->authorized_roles->contains($id)) ? 'selected' : '' }}>{{ $authorized_role }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('authorized_users') ? 'is-invalid' : '' }}" name="authorized_users[]" id="authorized_users" multiple>
                    @foreach($authorized_users as $id => $authorized_user)
                        <option value="{{ $id }}" {{ (in_array($id, old('authorized_users', [])) || $documentation->authorized_users->contains($id)) ? 'selected' : '' }}>{{ $authorized_user }}</option>
                    @endforeach
                </select>
