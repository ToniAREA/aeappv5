    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-employeeEmployeeAttendances">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $employeeAttendance->id }}">
                                ...
                                <td>
                                    @can('employee_attendance_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.employee-attendances.show', $employeeAttendance->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('employee_attendance_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.employee-attendances.edit', $employeeAttendance->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('employee_attendance_delete')
                                        <form action="{{ route('admin.employee-attendances.destroy', $employeeAttendance->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-employeeEmployeeAttendances:not(.ajaxTable)').DataTable({ buttons: dtButtons })
