<div class="m-3">
    @can('employees_skill_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-success" href="{{ route('admin.employees-skills.create') }}">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-employeeEmployeesSkills">
                    <thead>
                        <tr>
                            <th width="10">
                            ...
                            <tr data-entry-id="{{ $employeesSkill->id }}">
                                ...
                                <td>
                                    @can('employees_skill_show')
                                        <a class="btn btn-xs btn-primary" href="{{ route('admin.employees-skills.show', $employeesSkill->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan

                                    @can('employees_skill_edit')
                                        <a class="btn btn-xs btn-info" href="{{ route('admin.employees-skills.edit', $employeesSkill->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('employees_skill_delete')
                                        <form action="{{ route('admin.employees-skills.destroy', $employeesSkill->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('employees_skill_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.employees-skills.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-employeeEmployeesSkills:not(.ajaxTable)').DataTable({ buttons: dtButtons })
