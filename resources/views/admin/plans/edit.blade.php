        ...
        <form method="POST" action="{{ route("admin.plans.update", [$plan->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_online') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_online" id="is_online" value="1" {{ $plan->is_online || old('is_online', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_online">{{ trans('cruds.plan.fields.is_online') }}</label>
                </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $plan->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Plan::PERIOD_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('period') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="period_{{ $key }}" name="period" value="{{ $key }}" {{ old('period', $plan->period) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('hourly_rate') ? 'is-invalid' : '' }}" type="number" name="hourly_rate" id="hourly_rate" value="{{ old('hourly_rate', $plan->hourly_rate) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('hourly_rate_discount') ? 'is-invalid' : '' }}" type="number" name="hourly_rate_discount" id="hourly_rate_discount" value="{{ old('hourly_rate_discount', $plan->hourly_rate_discount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('material_discount') ? 'is-invalid' : '' }}" type="number" name="material_discount" id="material_discount" value="{{ old('material_discount', $plan->material_discount) }}" step="0.01">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('admin.plans.storeCKEditorImages') }}', true);
