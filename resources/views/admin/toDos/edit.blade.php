    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.to-dos.update", [$toDo->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('for_employee') ? 'is-invalid' : '' }}" name="for_employee_id" id="for_employee_id">
                    @foreach($for_employees as $id => $entry)
                        <option value="{{ $id }}" {{ (old('for_employee_id') ? old('for_employee_id') : $toDo->for_employee->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('priority') ? 'is-invalid' : '' }}" type="number" name="priority" id="priority" value="{{ old('priority', $toDo->priority) }}" step="1">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_repetitive') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_repetitive" id="is_repetitive" value="1" {{ $toDo->is_repetitive || old('is_repetitive', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('repeat_interval_value') ? 'is-invalid' : '' }}" type="number" name="repeat_interval_value" id="repeat_interval_value" value="{{ old('repeat_interval_value', $toDo->repeat_interval_value) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('repeat_interval_unit') ? 'is-invalid' : '' }}" name="repeat_interval_unit" id="repeat_interval_unit">
                    ...
                    @foreach(App\Models\ToDo::REPEAT_INTERVAL_UNIT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('repeat_interval_unit', $toDo->repeat_interval_unit) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('internal_notes') ? 'is-invalid' : '' }}" type="text" name="internal_notes" id="internal_notes" value="{{ old('internal_notes', $toDo->internal_notes) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('completed_at') ? 'is-invalid' : '' }}" type="text" name="completed_at" id="completed_at" value="{{ old('completed_at', $toDo->completed_at) }}">
