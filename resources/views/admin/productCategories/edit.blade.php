        ...
        <form method="POST" action="{{ route("admin.product-categories.update", [$productCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_online') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_online" id="is_online" value="1" {{ $productCategory->is_online || old('is_online', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="is_online">{{ trans('cruds.productCategory.fields.is_online') }}</label>
                </div>
            ...
            <div class="form-group">
                <label for="category_slug">{{ trans('cruds.productCategory.fields.category_slug') }}</label>
                <input class="form-control {{ $errors->has('category_slug') ? 'is-invalid' : '' }}" type="text" name="category_slug" id="category_slug" value="{{ old('category_slug', $productCategory->category_slug) }}">
                @if($errors->has('category_slug'))
                    <span class="text-danger">{{ $errors->first('category_slug') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.productCategory.fields.category_slug_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="description">{{ trans('cruds.productCategory.fields.description') }}</label>
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $productCategory->description) !!}</textarea>
            ...
            <div class="form-group">
                <label class="required" for="authorized_roles">{{ trans('cruds.productCategory.fields.authorized_roles') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('authorized_roles') ? 'is-invalid' : '' }}" name="authorized_roles[]" id="authorized_roles" multiple required>
                    @foreach($authorized_roles as $id => $authorized_role)
                        <option value="{{ $id }}" {{ (in_array($id, old('authorized_roles', [])) || $productCategory->authorized_roles->contains($id)) ? 'selected' : '' }}>{{ $authorized_role }}</option>
                    @endforeach
                </select>
                @if($errors->has('authorized_roles'))
                    <span class="text-danger">{{ $errors->first('authorized_roles') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.productCategory.fields.authorized_roles_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="authorized_users">{{ trans('cruds.productCategory.fields.authorized_users') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('authorized_users') ? 'is-invalid' : '' }}" name="authorized_users[]" id="authorized_users" multiple>
                    @foreach($authorized_users as $id => $authorized_user)
                        <option value="{{ $id }}" {{ (in_array($id, old('authorized_users', [])) || $productCategory->authorized_users->contains($id)) ? 'selected' : '' }}>{{ $authorized_user }}</option>
                    @endforeach
                </select>
                @if($errors->has('authorized_users'))
                    <span class="text-danger">{{ $errors->first('authorized_users') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.productCategory.fields.authorized_users_helper') }}</span>
            </div>
            <div class="form-group">
