            ...
            <div class="col-md-12">
                @can('product_tag_create')
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            <a class="btn btn-success" href="{{ route('frontend.product-tags.create') }}">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'ProductTag',
                                'route' => 'admin.product-tags.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                            ...
                                            <td>
                                                @can('product_tag_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.product-tags.show', $productTag->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('product_tag_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.product-tags.edit', $productTag->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('product_tag_delete')
                                                    <form
                                                        action="{{ route('frontend.product-tags.destroy', $productTag->id) }}"
                                                        method="POST"
        ...
        $(function() {
            ...
            @can('product_tag_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('frontend.product-tags.massDestroy') }}",
            ...
            let table = $('.datatable-ProductTag:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
