        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST"
                            action="{{ route('frontend.finantial-document-taxes.update', [$finantialDocumentTax->id]) }}"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="item_id" id="item_id">
                                    @foreach ($items as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('item_id') ? old('item_id') : $finantialDocumentTax->item->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('item'))
                                ...
                                @foreach (App\Models\FinantialDocumentTax::TAX_TYPE_RADIO as $key => $label)
                                    <div>
                                        <input type="radio" id="tax_type_{{ $key }}" name="tax_type"
                                            value="{{ $key }}"
                                            {{ old('tax_type', $finantialDocumentTax->tax_type) === (string) $key ? 'checked' : '' }}
                                            required>
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="number" name="tax_rate" id="tax_rate"
                                    value="{{ old('tax_rate', $finantialDocumentTax->tax_rate) }}" step="0.01">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="number" name="tax_amount" id="tax_amount"
                                    value="{{ old('tax_amount', $finantialDocumentTax->tax_amount) }}" step="0.01">
