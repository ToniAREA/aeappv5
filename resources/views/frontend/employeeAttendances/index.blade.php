                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                ...
                                'route' => 'admin.employee-attendances.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table
                                class=" table table-bordered table-striped table-hover datatable datatable-EmployeeAttendance">
                                <thead>
                                    <tr>
                                        <th>
                                            ...
                                            <td>
                                                @can('employee_attendance_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.employee-attendances.show', $employeeAttendance->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('employee_attendance_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.employee-attendances.edit', $employeeAttendance->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('employee_attendance_delete')
                                                    <form
                                                        action="{{ route('frontend.employee-attendances.destroy', $employeeAttendance->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-EmployeeAttendance:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
