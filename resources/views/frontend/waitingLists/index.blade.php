                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'WaitingList',
                                'route' => 'admin.waiting-lists.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                            ...
                                            <td>
                                                @can('waiting_list_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.waiting-lists.show', $waitingList->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('waiting_list_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.waiting-lists.edit', $waitingList->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('waiting_list_delete')
                                                    <form
                                                        action="{{ route('frontend.waiting-lists.destroy', $waitingList->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-WaitingList:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
