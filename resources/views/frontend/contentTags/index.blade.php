                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'ContentTag',
                                'route' => 'admin.content-tags.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                            ...
                                            <td>
                                                @can('content_tag_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.content-tags.show', $contentTag->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('content_tag_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.content-tags.edit', $contentTag->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('content_tag_delete')
                                                    <form
                                                        action="{{ route('frontend.content-tags.destroy', $contentTag->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-ContentTag:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
