            ...
            <div class="col-md-12">
                @can('asset_status_create')
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            <a class="btn btn-success" href="{{ route('frontend.asset-statuses.create') }}">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'AssetStatus',
                                'route' => 'admin.asset-statuses.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                            ...
                                            <td>
                                                @can('asset_status_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.asset-statuses.show', $assetStatus->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('asset_status_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.asset-statuses.edit', $assetStatus->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('asset_status_delete')
                                                    <form
                                                        action="{{ route('frontend.asset-statuses.destroy', $assetStatus->id) }}"
                                                        method="POST"
        ...
        $(function() {
            ...
            @can('asset_status_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('frontend.asset-statuses.massDestroy') }}",
            ...
            let table = $('.datatable-AssetStatus:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
