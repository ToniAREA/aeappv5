                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'Proforma',
                                'route' => 'admin.proformas.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                ...
                                <tbody>
                                    @foreach ($proformas as $key => $proforma)
                                        <tr data-entry-id="{{ $proforma->id }}">
                                            ...
                                            <td>
                                                <span
                                                    style="display:none">{{ $proforma->closed_and_protected ?? '' }}</span>
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $proforma->closed_and_protected ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $proforma->sent ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                                <span style="display:none">{{ $proforma->paid ?? '' }}</span>
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $proforma->paid ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                                {{ $proforma->claims ?? '' }}
                                            </td>
                                            ...
                                            <td>
                                                @can('proforma_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.proformas.show', $proforma->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('proforma_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.proformas.edit', $proforma->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan

                                                @can('proforma_delete')
                                                    <form action="{{ route('frontend.proformas.destroy', $proforma->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-Proforma:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
