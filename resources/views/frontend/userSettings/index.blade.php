            ...
            @can('user_setting_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        ...
                        @include('csvImport.modal', ['model' => 'UserSetting', 'route' => 'admin.user-settings.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                                    ...
                                    <tr data-entry-id="{{ $userSetting->id }}">
                                        ...
                                        <td>
                                            @can('user_setting_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-settings.show', $userSetting->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_setting_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-settings.edit', $userSetting->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_setting_delete')
                                                <form action="{{ route('frontend.user-settings.destroy', $userSetting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
