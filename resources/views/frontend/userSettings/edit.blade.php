        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('frontend.user-settings.update', [$userSetting->id]) }}"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="user_id" id="user_id">
                                    @foreach ($users as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('user_id') ? old('user_id') : $userSetting->user->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('user'))
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="title" id="title"
                                    value="{{ old('title', $userSetting->title) }}">
                                @if ($errors->has('title'))
                                    <div class="invalid-feedback">
                                        {{ $errors->first('title') }}
                                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="value" id="value">{{ old('value', $userSetting->value) }}</textarea>
                                @if ($errors->has('value'))
                                    <div class="invalid-feedback">
                                        {{ $errors->first('value') }}
                                    </div>
