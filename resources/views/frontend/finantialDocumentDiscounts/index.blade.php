            ...
            @can('finantial_document_discount_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        ...
                        @include('csvImport.modal', ['model' => 'FinantialDocumentDiscount', 'route' => 'admin.finantial-document-discounts.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-FinantialDocumentDiscount">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $finantialDocumentDiscount->id }}">
                                        ...
                                        <td>
                                            @can('finantial_document_discount_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.finantial-document-discounts.show', $finantialDocumentDiscount->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('finantial_document_discount_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.finantial-document-discounts.edit', $finantialDocumentDiscount->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('finantial_document_discount_delete')
                                                <form action="{{ route('frontend.finantial-document-discounts.destroy', $finantialDocumentDiscount->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-FinantialDocumentDiscount:not(.ajaxTable)').DataTable({ buttons: dtButtons })
