                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'WlistStatus',
                                'route' => 'admin.wlist-statuses.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                            ...
                                            <td>
                                                @can('wlist_status_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.wlist-statuses.show', $wlistStatus->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('wlist_status_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.wlist-statuses.edit', $wlistStatus->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('wlist_status_delete')
                                                    <form
                                                        action="{{ route('frontend.wlist-statuses.destroy', $wlistStatus->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-WlistStatus:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
