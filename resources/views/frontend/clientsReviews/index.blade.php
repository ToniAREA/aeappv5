            ...
            <div class="col-md-12">
                @can('clients_review_create')
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            <a class="btn btn-success" href="{{ route('frontend.clients-reviews.create') }}">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'ClientsReview',
                                'route' => 'admin.clients-reviews.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table
                                class=" table table-bordered table-striped table-hover datatable datatable-ClientsReview">
                                <thead>
                                    <tr>
                                        <th>
                                        ...
                                        <tr data-entry-id="{{ $clientsReview->id }}">
                                            ...
                                            <td>
                                                ...
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $clientsReview->shown_online ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                                @can('clients_review_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.clients-reviews.show', $clientsReview->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('clients_review_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.clients-reviews.edit', $clientsReview->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('clients_review_delete')
                                                    <form
                                                        action="{{ route('frontend.clients-reviews.destroy', $clientsReview->id) }}"
                                                        method="POST"
        ...
        $(function() {
            ...
            @can('clients_review_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('frontend.clients-reviews.massDestroy') }}",
            ...
            let table = $('.datatable-ClientsReview:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
