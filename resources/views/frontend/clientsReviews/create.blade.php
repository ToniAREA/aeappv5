    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.clients-reviews.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="for_wlists[]" id="for_wlists" multiple>
                                @foreach($for_wlists as $id => $for_wlist)
                                    <option value="{{ $id }}" {{ in_array($id, old('for_wlists', [])) ? 'selected' : '' }}>{{ $for_wlist }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="rating" id="rating" value="{{ old('rating', '') }}" step="0.1">
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="shown_online" id="shown_online" value="1" {{ old('shown_online', 0) == 1 ? 'checked' : '' }}>
                                <label for="shown_online">{{ trans('cruds.clientsReview.fields.shown_online') }}</label>
                            </div>
