                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.faq-categories.update", [$faqCategory->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $faqCategory->description) }}">
                        ...
                        <div class="form-group">
                            <label for="authorized_roles">{{ trans('cruds.faqCategory.fields.authorized_roles') }}</label>
                            ...
                            <select class="form-control select2" name="authorized_roles[]" id="authorized_roles" multiple>
                                @foreach($authorized_roles as $id => $authorized_role)
                                    <option value="{{ $id }}" {{ (in_array($id, old('authorized_roles', [])) || $faqCategory->authorized_roles->contains($id)) ? 'selected' : '' }}>{{ $authorized_role }}</option>
                                @endforeach
                            </select>
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.faqCategory.fields.authorized_roles_helper') }}</span>
                        </div>
                        <div class="form-group">
                            <label for="authorized_users">{{ trans('cruds.faqCategory.fields.authorized_users') }}</label>
                            <div style="padding-bottom: 4px">
                                <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                                <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                            </div>
                            <select class="form-control select2" name="authorized_users[]" id="authorized_users" multiple>
                                @foreach($authorized_users as $id => $authorized_user)
                                    <option value="{{ $id }}" {{ (in_array($id, old('authorized_users', [])) || $faqCategory->authorized_users->contains($id)) ? 'selected' : '' }}>{{ $authorized_user }}</option>
                                @endforeach
                            </select>
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.faqCategory.fields.authorized_users_helper') }}</span>
                        </div>
                        <div class="form-group">
