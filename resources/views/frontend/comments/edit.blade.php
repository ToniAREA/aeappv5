        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('frontend.comments.update', [$comment->id]) }}"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="wlist_id" id="wlist_id">
                                    @foreach ($wlists as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('wlist_id') ? old('wlist_id') : $comment->wlist->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('wlist'))
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="from_user_id" id="from_user_id">
                                    @foreach ($from_users as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('from_user_id') ? old('from_user_id') : $comment->from_user->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            return loader.file
                                .then(function(file) {
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        xhr.open('POST',
                                            '{{ route('frontend.comments.storeCKEditorImages') }}',
                                            true);
