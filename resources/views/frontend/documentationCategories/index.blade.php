                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                ...
                                'route' => 'admin.documentation-categories.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table
                                class=" table table-bordered table-striped table-hover datatable datatable-DocumentationCategory">
                                <thead>
                                    <tr>
                                        <th>
                                ...
                                <tbody>
                                    @foreach ($documentationCategories as $key => $documentationCategory)
                                        <tr data-entry-id="{{ $documentationCategory->id }}">
                                            ...
                                            <td>
                                                <span
                                                    style="display:none">{{ $documentationCategory->is_online ?? '' }}</span>
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $documentationCategory->is_online ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                @if ($documentationCategory->photo)
                                                    <a href="{{ $documentationCategory->photo->getUrl() }}" target="_blank"
                                            ...
                                            <td>
                                                @can('documentation_category_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.documentation-categories.show', $documentationCategory->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('documentation_category_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.documentation-categories.edit', $documentationCategory->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('documentation_category_delete')
                                                    <form
                                                        action="{{ route('frontend.documentation-categories.destroy', $documentationCategory->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-DocumentationCategory:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
