                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'Claim',
                                'route' => 'admin.claims.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                            ...
                                            <td>
                                                @can('claim_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.claims.show', $claim->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('claim_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.claims.edit', $claim->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan

                                                @can('claim_delete')
                                                    <form action="{{ route('frontend.claims.destroy', $claim->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-Claim:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
