            ...
            <div class="col-md-12">
                @can('booking_slot_create')
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            <a class="btn btn-success" href="{{ route('frontend.booking-slots.create') }}">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'BookingSlot',
                                'route' => 'admin.booking-slots.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                        ...
                                        <td>
                                            <select class="search">
                                                ...
                                                @foreach ($employees as $key => $item)
                                                    <option value="{{ $item->id_employee }}">{{ $item->id_employee }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </td>
                                            ...
                                            <td>
                                                @can('booking_slot_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.booking-slots.show', $bookingSlot->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('booking_slot_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.booking-slots.edit', $bookingSlot->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('booking_slot_delete')
                                                    <form
                                                        action="{{ route('frontend.booking-slots.destroy', $bookingSlot->id) }}"
                                                        method="POST"
        ...
        $(function() {
            ...
            @can('booking_slot_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('frontend.booking-slots.massDestroy') }}",
            ...
            let table = $('.datatable-BookingSlot:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
