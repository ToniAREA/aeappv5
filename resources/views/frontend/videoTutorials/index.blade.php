                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'VideoTutorial',
                                'route' => 'admin.video-tutorials.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table
                                class=" table table-bordered table-striped table-hover datatable datatable-VideoTutorial">
                                <thead>
                                    <tr>
                                        <th>
                                        ...
                                        <tr data-entry-id="{{ $videoTutorial->id }}">
                                            ...
                                            <td>
                                                ...
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $videoTutorial->is_online ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                                {{ $videoTutorial->title ?? '' }}
                                            </td>
                                            ...
                                            <td>
                                                @if ($videoTutorial->image)
                                                    <a href="{{ $videoTutorial->image->getUrl() }}" target="_blank"
                                            ...
                                            <td>
                                                @can('video_tutorial_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.video-tutorials.show', $videoTutorial->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('video_tutorial_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.video-tutorials.edit', $videoTutorial->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('video_tutorial_delete')
                                                    <form
                                                        action="{{ route('frontend.video-tutorials.destroy', $videoTutorial->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-VideoTutorial:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
