            ...
            <div class="col-md-12">
                @can('maintenance_suscription_create')
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            <a class="btn btn-success" href="{{ route('frontend.maintenance-suscriptions.create') }}">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'MaintenanceSuscription',
                                'route' => 'admin.maintenance-suscriptions.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table
                                class=" table table-bordered table-striped table-hover datatable datatable-MaintenanceSuscription">
                                <thead>
                                    <tr>
                                        <th>
                                ...
                                <tbody>
                                    @foreach ($maintenanceSuscriptions as $key => $maintenanceSuscription)
                                        <tr data-entry-id="{{ $maintenanceSuscription->id }}">
                                            ...
                                            <td>
                                                <span
                                                    style="display:none">{{ $maintenanceSuscription->is_active ?? '' }}</span>
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $maintenanceSuscription->is_active ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                @if ($maintenanceSuscription->signed_contract)
                                                    <a href="{{ $maintenanceSuscription->signed_contract->getUrl() }}"
                                                        target="_blank">
                                            ...
                                            <td>
                                                @can('maintenance_suscription_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.maintenance-suscriptions.show', $maintenanceSuscription->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('maintenance_suscription_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.maintenance-suscriptions.edit', $maintenanceSuscription->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('maintenance_suscription_delete')
                                                    <form
                                                        action="{{ route('frontend.maintenance-suscriptions.destroy', $maintenanceSuscription->id) }}"
                                                        method="POST"
        ...
        $(function() {
            ...
            @can('maintenance_suscription_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('frontend.maintenance-suscriptions.massDestroy') }}",
            ...
            let table = $('.datatable-MaintenanceSuscription:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
