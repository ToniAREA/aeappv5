                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'CarePlan',
                                'route' => 'admin.care-plans.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                ...
                                <tbody>
                                    @foreach ($carePlans as $key => $carePlan)
                                        <tr data-entry-id="{{ $carePlan->id }}">
                                            ...
                                            <td>
                                                @if ($carePlan->photo)
                                                    <a href="{{ $carePlan->photo->getUrl() }}" target="_blank"
                                            ...
                                            <td>
                                                @can('care_plan_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.care-plans.show', $carePlan->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('care_plan_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.care-plans.edit', $carePlan->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan

                                                @can('care_plan_delete')
                                                    <form action="{{ route('frontend.care-plans.destroy', $carePlan->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-CarePlan:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
