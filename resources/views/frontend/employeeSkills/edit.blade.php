        ...
        <div class="row justify-content-center">
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('frontend.employee-skills.update', [$employeeSkill->id]) }}"
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="employee_id" id="employee_id">
                                    @foreach ($employees as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('employee_id') ? old('employee_id') : $employeeSkill->employee->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2" name="subject_id" id="subject_id">
                                    @foreach ($subjects as $id => $entry)
                                        <option value="{{ $id }}"
                                            {{ (old('subject_id') ? old('subject_id') : $employeeSkill->subject->id ?? '') == $id ? 'selected' : '' }}>
                                            {{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="number" name="level" id="level"
                                    value="{{ old('level', $employeeSkill->level) }}" step="1">
                                @if ($errors->has('level'))
                                    <div class="invalid-feedback">
                                        {{ $errors->first('level') }}
                                    </div>
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control" type="text" name="description" id="description"
                                    value="{{ old('description', $employeeSkill->description) }}">
                                ...
                                @endif
                                <span
                                    class="help-block">{{ trans('cruds.employeeSkill.fields.description_helper') }}</span>
                            </div>
                            <div class="form-group">
                                <div>
                                    <input type="hidden" name="verified" value="0">
                                    <input type="checkbox" name="verified" id="verified" value="1"
                                        {{ $employeeSkill->verified || old('verified', 0) === 1 ? 'checked' : '' }}>
                            ...
                            <div class="form-group">
                                <label
                                    for="internal_notes">{{ trans('cruds.employeeSkill.fields.internal_notes') }}</label>
                                <input class="form-control" type="text" name="internal_notes" id="internal_notes"
                                    value="{{ old('internal_notes', $employeeSkill->internal_notes) }}">
                                ...
                                @endif
                                <span
                                    class="help-block">{{ trans('cruds.employeeSkill.fields.internal_notes_helper') }}</span>
                            </div>
                            <div class="form-group">
