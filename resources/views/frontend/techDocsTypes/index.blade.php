                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'TechDocsType',
                                'route' => 'admin.tech-docs-types.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                                ...
                                <tbody>
                                    @foreach ($techDocsTypes as $key => $techDocsType)
                                        <tr data-entry-id="{{ $techDocsType->id }}">
                                            ...
                                            <td>
                                                <span style="display:none">{{ $techDocsType->is_online ?? '' }}</span>
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $techDocsType->is_online ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                                {{ $techDocsType->name ?? '' }}
                                            </td>
                                            <td>
                                                @if ($techDocsType->photo)
                                                    <a href="{{ $techDocsType->photo->getUrl() }}" target="_blank"
                                            ...
                                            <td>
                                                @can('tech_docs_type_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.tech-docs-types.show', $techDocsType->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('tech_docs_type_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.tech-docs-types.edit', $techDocsType->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('tech_docs_type_delete')
                                                    <form
                                                        action="{{ route('frontend.tech-docs-types.destroy', $techDocsType->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-TechDocsType:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
