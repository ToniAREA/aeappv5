            ...
            <div class="col-md-12">
                @can('financial_document_item_create')
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            <a class="btn btn-success" href="{{ route('frontend.financial-document-items.create') }}">
                            ...
                            @include('csvImport.modal', [
                                ...
                                'route' => 'admin.financial-document-items.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table
                                class=" table table-bordered table-striped table-hover datatable datatable-FinancialDocumentItem">
                                <thead>
                                    <tr>
                                        <th>
                                            ...
                                            <td>
                                                @can('financial_document_item_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.financial-document-items.show', $financialDocumentItem->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('financial_document_item_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.financial-document-items.edit', $financialDocumentItem->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('financial_document_item_delete')
                                                    <form
                                                        action="{{ route('frontend.financial-document-items.destroy', $financialDocumentItem->id) }}"
                                                        method="POST"
        ...
        $(function() {
            ...
            @can('financial_document_item_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('frontend.financial-document-items.massDestroy') }}",
            ...
            let table = $('.datatable-FinancialDocumentItem:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
