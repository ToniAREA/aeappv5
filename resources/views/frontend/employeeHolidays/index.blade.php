            ...
            <div class="col-md-12">
                @can('employee_holiday_create')
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            <a class="btn btn-success" href="{{ route('frontend.employee-holidays.create') }}">
                            ...
                            @include('csvImport.modal', [
                                ...
                                'route' => 'admin.employee-holidays.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table
                                class=" table table-bordered table-striped table-hover datatable datatable-EmployeeHoliday">
                                <thead>
                                    <tr>
                                        <th>
                                ...
                                <tbody>
                                    @foreach ($employeeHolidays as $key => $employeeHoliday)
                                        <tr data-entry-id="{{ $employeeHoliday->id }}">
                                            ...
                                            <td>
                                                <span
                                                    style="display:none">{{ $employeeHoliday->is_completed ?? '' }}</span>
                                                <input type="checkbox" disabled="disabled"
                                                    {{ $employeeHoliday->is_completed ? 'checked' : '' }}>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                @can('employee_holiday_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.employee-holidays.show', $employeeHoliday->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('employee_holiday_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.employee-holidays.edit', $employeeHoliday->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('employee_holiday_delete')
                                                    <form
                                                        action="{{ route('frontend.employee-holidays.destroy', $employeeHoliday->id) }}"
                                                        method="POST"
        ...
        $(function() {
            ...
            @can('employee_holiday_delete')
                ...
                let deleteButton = {
                    ...
                    url: "{{ route('frontend.employee-holidays.massDestroy') }}",
            ...
            let table = $('.datatable-EmployeeHoliday:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
